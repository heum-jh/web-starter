...
type MeetingMemberListCardProps = {
  role?: "user" | "member" | "writer";
  onExile?: () => void;
  onApprove?: () => void;
  onReject?: () => void;
};
const MeetingMemberListCard = ({ role = "member", onExile, onApprove, onReject }: MeetingMemberListCardProps) => {
  return (
    <li className="flex w-full items-center gap-x-[0.62rem]">
      ...
      {role === "writer" && (
        <button type="button" className="ml-auto text-base font-medium text-[#F2555A]" onClick={onExile}>
          내보내기
        </button>
      )}
      {role === "user" && (
        <div className="ml-auto space-x-2">
          <button
            type="button"
            className="rounded border border-[#DBDEE4] px-2 py-[0.38rem] text-sm font-medium text-[#707888]"
            onClick={onReject}
          >
            거절
          </button>
          <button
            type="button"
            className="rounded bg-[#258DED] px-2 py-[0.38rem] text-sm font-medium text-white"
            onClick={onApprove}
          >
            승인
          </button>
...
export default MeetingMemberListCard;
